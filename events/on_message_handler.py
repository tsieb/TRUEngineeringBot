...
async def on_message_handler__(client, message):
  ...
  if (await response_handler.command_handler(client, message)):
    return
  if (await response_handler.private_response_handler(client, message = message)):
    return
  if (await response_handler.global_response_handler(client, message = message)):
    return
