...
async def welcome_handler(client, message = None, payload = None, member = None):
  if member:
    user = functions.get_user(member.id)
  elif payload:
    user = functions.get_user(payload.user_id)
  elif message:
  ...
  if(not("welcome" in user["response"])):
    ...
    return False
  if (user["response"]["welcome"]["conv_state"] == 0 and payload):
    for i in welcome_get_reaction["reaction"]:
      if ([f"0x{ord(c):08x}" for c in payload.emoji.name] == i[1]):
        if (i[2] != None):
          for i in i[2]: user["purpose"].append(i[2])
    if (user["purpose"] != [] and user["purpose"] != ["other"]):
...
async def welcome_send_message(client, user):
  ...
  for emoji in welcome_get_reaction()[0]:
